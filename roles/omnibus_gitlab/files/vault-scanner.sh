...
files_found=$(find "${TEMPDIR}" -iname secrets.yml -o -iname "*vault*.yml")
...
# search for certain file names to avoid committing
files_found=$(find "${TEMPDIR}" -iname vault_password_file)
for f in ${files_found}; do
        if [[ $(basename "$f") == "vault_password_file" ]]; then
                  echo "ERROR: $(basename "$f") should not be committed"
                  bad_file=1
        fi
done
